...
public class Slot : MonoBehaviour,IPointerEnterHandler,IPointerExitHandler,IPointerDownHandler{
    ...
    public bool isFiled()
    {
        ...
        return itemUI.Amount >= itemUI.Item.Capaticy; //true表示当前数量大于等于容量，false表示当前数量小于容量
    }
    //接口实现的方法，鼠标覆盖时触发
        ...
        if (transform.childCount > 0) //二：自身不是空
        {
            ...
            else
            {
                ...
                if (currentItem.Item.Id == InventroyManager.Instance.PickedItem.Item.Id)
                {
                    ...
                    {
                        if (currentItem.Item.Capaticy > currentItem.Amount)//如果物品槽里的物品容量大于当前物品数量，那就说明物品槽里还能再放
                        {
                    ...
                    else
                    {
                        if (currentItem.Item.Capaticy > currentItem.Amount)//如果物品槽里的物品容量大于当前物品数量，那就说明物品槽里还能再放
                        {
                            int itemRemain = currentItem.Item.Capaticy - currentItem.Amount;//物品槽还可以再放的数量
                ...
                else//②自身的id != pickedItem.id，pickedItem跟当前物品交换
                {
                    ...
                    ItemData pickedItemTemp = InventroyManager.Instance.PickedItem.Item;
                    int pickedItemAmountTemp = InventroyManager.Instance.PickedItem.Amount;

                    //保存当前物品槽中的物品，用于和鼠标上的物品交换
                    ItemData currentItemTemp = currentItem.Item;
